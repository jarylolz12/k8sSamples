        ...
        <div class="modal">
            ...
            <div class="flex flex-col button-modal-container">
                <div class="button-main-container">
                    ...
                    <button @click="connectToBuyer" class="btn btn-default bg-success text-white py-0 leading-3 btn-yes">
...
<script>
    ...
    export default {
        props: ['resource', 'resourceName', 'resourceId', 'field', 'buyerId', 'customerId', 'connectedCustomerId', 'customerName', 'supplierId'],
        data() {
            return {
                sending: false,
                showModal: false,
        ...
        methods: {
            ...
            disConnectToCustomer(id) {
                const params = {
                    id: this.field.buyerId,
                    supplierId: this.field.supplierId,
                    currentCustomerId: this.field.customerId,
                    customerId: this.field.connectedCustomerId,
                };
                Nova.request().post(
                    `/custom/buyers/disconnect`
                    , params);
                location.reload();
            },
            connectToBuyer: async function (data) {
                const params = {selectedCustomerId: this.item.value, customerId: this.field.customerId, buyerId: this.field.buyerId};
                await Nova.request().post(
                    `/custom/buyers/connect`
                    , params);
