...
class ProfitMonitorController extends Controller
{
    ...
    /**
     * @authenticated
     * @group Profit Monitor
     *
     * Get calculated shipment V2
     *
     * @queryParam per-page int
     * @queryParam filterbyrefnum int
     * @queryParam filterbycustomer string
     * @queryParam realmId int
     * @queryParam sortby string
     * @queryParam sorttype string
     * @queryParam month string. Example: April 27, 2022
     * @queryParam sale_persons string
    ...
    {
//
//        $authRole = Auth::user()->whereHas('roles', function($role){
...
//        dd($authRole);
      ...
      //  $authRole = Auth::loginUsingId(11);
            if(strtoupper($authRole->name) === "SUPER ADMIN" || strtoupper($authRole->name) === "SALES REPRESENTATIVE" ){
                ...
                ////// PROFIT FILTER //////
                // $profit_filter = $request->input('profit_filter');

                // $filtered_profit_ids = [];

                // if( !empty($profit_filter) ){
                //     $collection = Shipment::select('id')->get();
                //     $filtered_profits_ids = $collection->filter(function($model) use($profit_filter){
                //         if( $profit_filter == 'less than 50' ){
                //             return $model->Profitt < 50;
                //         }elseif( $profit_filter == 'profit less than projected' ){
                //             return $model->Profitt < $model->projected_profit;
                //         }                        
                //     })
                //     ->all()
                //     ->pluck('id');
                ...
                if (count($shipments) > 0) {
                    ...
                    foreach($shipments as $key => $shipment) {
                        $sort_array['profit'][$key] =$shipment['profitt'];
                        $sort_array['shifl_ref'][$key] = $shipment['shifl_ref'];
                        $sort_array['customer'][$key] = $shipment['eta'];
                        $sort_array['projected_profit'][$key] = $shipment['projected_profit'];
                        $sort_array['eta'][$key] = $shipment['eta'];
                    }

                    $sortables = ['profit','shifl_ref', 'customer', 'projected_profit', 'eta'];

                    foreach($sortables as $sortable) {
                        if ($sortable === $sort_by) {
                            array_multisort($sort_array[$sortable], ($sort_type=='asc') ? SORT_ASC : SORT_DESC, $shipments);
                        }
                    }
                }
                ...
                // if (empty($request->per_page)) {
                    return StandardResource::collection((new Collection($shipments))->paginate($request->query('per-page',50)));
                // }
                // if (is_numeric($request->per_page)) {
                    return StandardResource::collection((new Collection($shipments))->paginate($request->per_page));
                // }
                //return response()->json($response);
            }
     ...
     * @group Profit Monitor
     *
     * Get calculated shipment v2 bak
     *
     *
     * @param Request $request
    ...
    public function getCalculatedShipmentV2Bak(Request $request)
    {
        $authRole = Auth::user()->roles()->first();
        if(strtoupper($authRole->name) === "SUPER ADMIN" || strtoupper($authRole->name) === "SALES REPRESENTATIVE" ){
            $response = ['status' => 'ok','results' => []];
