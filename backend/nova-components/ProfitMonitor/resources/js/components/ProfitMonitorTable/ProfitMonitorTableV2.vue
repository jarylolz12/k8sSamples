...
<div class="flex w-full mt-4">
    <div class="w-full">        
        <div class="flex w-full mb-2 pl-2 pr-2">
            <div class="flex-auto">
                <span class="uppercase text-90 text-2xl font-bold mr-4">Total Profit:</span><span class="text-90 text-xl font-semibold">{{ moneyFormat(totalProfit ? parseFloat(totalProfit).toFixed(2) : parseFloat(0).toFixed(2),'USD').replace('$','') }}</span>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                {
                    label: 'Containers Qty',
                    field: 'containers_qty',
                    filterOptions: {
                        enabled: false,
                    },
                    sortable: true,
    ...
    watch: {
        ...
        profit_filter(v){
            this.rows = this.items2;
            if( v == 'less than 50' ){
                this.rows = this.rows.filter( i => i.profitt < 50 );
            }else if( v == 'profit less than projected'){
                this.rows = this.rows.filter( i => i.profitt < i.projected_profit );
            }
        }
    },
    methods: {
        ...
        fetchShipment() {
            ...
            .then(response => {
                ...
                let result = response.data.message.data.map( i =>{
                    i.profit = i.profitt;
                    return i;
                });
                ...
                if (Object.keys(result).length > 0) {
                    ...
                    let service_amt = 0;
                        ...
                        rowData.push({
                            ...
                            id: o.id,
                            containers_qty: o.containers_qty,
                ...
                _this.totalRecords = response.data.message.total;

                _this.rows = rowData;
                _this.items2 = rowData;
                // _this.totalProfit = parseFloat(response.data.data.invoices) - parseFloat(response.data.data.bills);

                if( v == 'less than 50' ){
                    _this.rows = _this.rows.filter( i => i.profitt < 50 );
                }else if( v == 'profit less than projected'){
                    _this.rows = _this.rows.filter( i => i.profitt < i.projected_profit );
                }
        ...
        getProfit(){ 
            ...
            .then(response => {
                ...
                _this.totalProfit = parseFloat(invoices) - parseFloat(bills);
