...
Nova.booting(function (Vue, router, store) {
  Vue.component('index-po-item-field', __webpack_require__(14));
  Vue.component('detail-po-item-field', __webpack_require__(17));
  Vue.component('form-po-item-field', __webpack_require__(20));
...
var update = __webpack_require__(24)("61ca5373", content, false, {});
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_laravel_nova__ = __webpack_require__(27);
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    itemsOption: function itemsOption() {
      if (this.selectedItems.length > 0) {
        var selected = _.reject(this.selectedItems, { item: null });
        var options = _.differenceWith(this.customerItems, _.map(selected, function (_ref) {
          var id = _ref.item;
          return id;
        }), function (a, b) {
          return a.id === b.id;
  ...
  methods: {
    ...
    fill: function fill(formData) {
      if (this.selectedItems.length > 0) {
        var getFieldValue = _.filter(this.selectedItems, function (c) {
          if (c.item !== null && c.quantity !== null) if (c.item !== null || c.quantity !== null) return c;
        });
    ...
    addFieldGroup: function addFieldGroup() {
      if (this.customer) {
        var hasEmptyVal = _.filter(this.selectedItems, { item: null }).length > 0;
    ...
    removeFieldGroup: function removeFieldGroup(item, key) {
      if (item) {
        var filteredItemFields = _.filter(this.selectedItems, function (c) {
  ...
  mounted: function mounted() {
    ...
    Nova.$on('nova-belongsto-depend-customer', function (data) {
      _this.customer = data;
    });

    if (this.resourceId) {
      console.log(this.resourceId);
      __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get('/custom/purchase-order-by-id/' + this.resourceId, {
        headers: {
          'x-csrf-token': document.querySelectorAll('meta[name=csrf-token]')[0].getAttributeNode('content').value,
          'Accept': 'application/json'
        }
      }).then(function (res) {
        var customer = res.data.results ? res.data.results[0].customer : '';
        console.log(customer);
        _this.customer = { value: customer };
      }).catch(function (err) {
        console.log(err);
      });
    }
  ...
  watch: {
    customer: function customer(newVal, OldVal) {
      ...
      if (newVal.value.id) {
        __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get('/custom/items-by-customer/' + newVal.value.id, {
          headers: {
            'x-csrf-token': document.querySelectorAll('meta[name=csrf-token]')[0].getAttributeNode('content').value,
            'Accept': 'application/json'
          }
          ...
          var responseResults = _.map(res.data.results, function (c) {
            return _.pick(c, ['id', 'item_num']);
          });
          _this2.customerItems = res.data ? responseResults : [];
        }).catch(function (err) {
          console.log(err);
        });
      }
      ...
      _c(
        ...
        [
          _vm._l(_vm.selectedItems, function(item, key) {
            return _c(
              "div",
              {
                ...
                staticClass: "w-full",
                attrs: {
                  id: "po-item-field-" + (item.item ? item.item.id : null)
                }
              },
              [
                _c("div", { staticClass: "w-4/5" }, [
                  ...
                  _c("div", [
                    _c(
                      ...
                      {
                        staticClass:
                          "mt-2 btn btn-sm btn-default btn-danger inline-flex items-center relative",
                        on: {
                          click: function($event) {
                            $event.preventDefault()
                            return _vm.removeFieldGroup(item.item, key)
                          }
                        }
                      },
                      [_vm._v("Remove")]
          ...
          _c(
            "div",
            {
              class: _vm.selectedItems.length > 0 ? "mt-2" : "",
              attrs: { id: "shipment-supplier-group-submit-group" }
            },
            [
              _c(
                "button",
                {
                  staticClass:
                    "btn btn-default btn-primary inline-flex items-center relative mr-3 shipment-add-group",
                  on: {
                    click: function($event) {
                      $event.preventDefault()
                      return _vm.addFieldGroup()
                    }
                  }
                },
                [_vm._v("Add Item")]
