...
class StatsController extends Controller{
	...
	public function graph(Request $request){
		...
		foreach( $terminals as $t ){
			$labels = $request->input('staging') == 1 ? $this->getGraphLabels2($filter,$t['name']) : $this->getGraphLabels($filter,$t['name']);
	...
	*/
	...
	public function getGraphLabels($filter,$terminal){
		...
		if( $filter == '18 Months' ){
			...
			$first = DB::table('index_rates')
			->select('data_date')
			->whereRaw('UPPER(terminal) = "'.strtoupper($terminal).'"')
			->orderBy('data_date')
			->first();
	...
	public function getGraphLabels2($filter,$terminal){
		...
		if( $from ){
			...
			->select('data_date')
			->whereRaw('UPPER(terminal) = "'.strtoupper($terminal).'"')
			->whereBetween('data_date',[$from,$arr[count($arr)-1]] )
			->groupBy('data_date')
			->get()
			->pluck('data_date');
		}
