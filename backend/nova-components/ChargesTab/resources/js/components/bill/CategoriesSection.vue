    ...
    <div>
        ...
        <div class="items-container mt-1">
            <div :id="`category-group-${item.id}`" v-for="(item,key) in itemGroup" :key="item.key" class="flex mt-2">
                <div class="inline-block mr-1" style="width:40%">
                    <div>
                        <v-select
                            id="categorySelect"
        ...
        <div class="flex flex-col mt-4 mb-4" v-if="itemGroup.length > 0">
            ...
            <!-- <div class="total-amount-label flex float-right font-semibold" style="margin-right:85px">
                <label class="mr-2">Total {{totalAmount ? parseFloat(totalAmount).toFixed(2) : 0}}</label>
            </div> -->
        </div>
    </div>
</template>
...
export default {
    ...
    props: ["resourceId","initCategorySectionValue","exchangeRateValue"],
