...
export default {
    name: "EditCategoriesGrid",
    components: {
        JqxGrid,
        JqxButton
    },
    props: ["gridCustomers", "resourceId", 'expenseAccounts','initGridValue'],
    data() {
        return {
            items: [],
            customer: 0,
    ...
    beforeCreate: function () {
        ...
        this.source = {
            ...
            datafields: [
                ...
                { name: 'Customer', values: { source: this.gridCustAdapter.records, value: 'Id', name: 'FullyQualifiedName' } },
            ],
            id: 'id',
    ...
    created() {
        console.log('this.initGridValue created');
        console.log(this.initGridValue);
        
    },
    beforeMount() {
       console.log('this.initGridValue beforemount');
        console.log(this.initGridValue);
    },
    methods: {
    ...
    mounted() {
        ...
        this.createButtons();
        const initialData = [];
        if(this.initGridValue){
            this.initGridValue.map(function(val){
                let row = {};
                row['Category'] = val.qbo_item_name;
                row['CategoryId'] = val.qbo_item_value;
                row['Customer'] = val.qbo_customer_name;
                row['CustomerId'] = val.qbo_customer_id;
                row['Description'] = val.qbo_description;
                row['Amount'] = val.qbo_amount;
                initialData.push(row)
            })
        }
        //pass value of initial Data to gridValues;

        console.log('this.initGridValue created');
        console.log(initialData);

        this.$refs.categoryGrid.addrow(null,initialData);
    }
};
</script>
