...
namespace Vishalmarakana\ShipmentOperations\Http\Controllers;
...
class ShipmentOperationsController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(!is_null($request->shiflOfficeTo)){
            if($request->shiflOfficeTo != 'all') $shipment_query->where('shipments.shifl_office_origin_to_id', $request->shiflOfficeTo);
        }
        if(!empty($request->shipmentStatus)){
            switch ($request->shipmentStatus) {
                case 'Pending Approval':
                    return $shipment_query->where('booking_confirmed', 0)
                        ->where('cancelled', 0);
                default:
                    return $shipment_query->where('shipment_status', $request->shipmentStatus);
            }
        }
