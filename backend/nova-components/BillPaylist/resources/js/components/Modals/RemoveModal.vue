<template>
    <div @click="closeModal" class="modal-overlay">
        <div :class="`modal remove-modal ${removeClass}`">
            <h6 class="text-white py-16">{{ title }}</h6>
            <slot name="content" v-bind:getItem="item"></slot>
            <div class="btn-group-div" v-if="!removing">
...
export default {
    components:{},
    props: ['modal', 'title', 'item', 'removing', 'removeClass'],
    mounted() {},
    methods: {
        closeModal() {
            this.$emit('update:removeClass', 'slide-out-elliptic-top-bck')
            setTimeout(() => {
              this.$emit('update:removeClass', '')
