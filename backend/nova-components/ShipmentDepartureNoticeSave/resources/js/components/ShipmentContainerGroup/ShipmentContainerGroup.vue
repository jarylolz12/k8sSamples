    ...
    <default-field :field="field" :errors="errors">
        <template slot="field">
            <input
                ...
                v-model="value"
            />
            <div :id="`shipment-container-group-${item.id}`" v-for="(item,key) in formGroups" class="shipment-container-group" style="border-bottom: 1px solid #eef1f4; padding-top: 4px;">
                <div>
                ...
                <div>
                    <label>Seal #</label>
                    <input type="text" v-model="item.seal_num" class="w-full form-control form-input form-input-bordered" />
                </div>
                <div>
                    <label>Sizes</label>
...
export default {
    ...
    props: ['resourceName', 'resourceId', 'field', 'updateContainerGroup', 'sizes'],
    updated() {
        this.updateContainerGroup(this.formGroups)
    },
    data: function() {
        return {
            errors: {},
