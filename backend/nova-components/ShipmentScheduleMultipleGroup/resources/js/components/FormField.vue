    ...
    <default-field :field="field" :errors="errors">
        <template slot="field">
            <input
                ...
                v-model="value"
            />
            <div :id="`shipment-schedule-multiple-group-${item.id}`" v-for="(item,key) in formGroups" class="shipment-schedule-multiple-group">
                ...
                <div>
                 ...
                 <div>
                    <button class="btn btn-default btn-danger inline-flex items-center relative mr-3 shipment-add-group" @click.prevent="removeGroup(item,key)" >Remove Schedule</button>
                </div>
            </div>
            <div id="shipment-schedule-multiple-group-submit-group">
                <button class="btn btn-default btn-primary inline-flex items-center relative mr-3 shipment-add-group" @click.prevent="addGroup()" >Add Leg</button>
            </div>
...
export default {
    ...
    methods: {
        removeGroup(item,key) {
            jQuery(`#shipment-schedule-multiple-group-${key}`).remove();
