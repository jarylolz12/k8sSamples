...
export default {
    ...
    mounted() {
        ...
        if(this.field.value!='') {
            var schedules = null
            if (typeof this.field.shipment.schedules_group_bookings!=='undefined' ) {
                schedules = JSON.parse(this.field.shipment.schedules_group_bookings);
            } else {
                schedules = JSON.parse(this.field.value);
            }
