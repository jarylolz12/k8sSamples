...
class SendBookingEmailEventListener
{
    /**
     ...
     * @return void
     */
    public function handle(SendBookingEmailEvent $event)
    {
        //\Debugbar::info('hello');
            ...
            foreach ($newFinalSchedules as $key => $newFinalSchedule) {
                ...
                if (isset($newFinalSchedule->sell_rates) && $newFinalSchedule->sell_rates!==null && $newFinalSchedule->sell_rates!=='' && count($newFinalSchedule->sell_rates) > 0) {
                    ...
                    foreach ($newFinalSchedule->sell_rates as $keySecond => $sell_rate) {
                        $totalAmount = $totalAmount + (floatval($sell_rate->qty) * floatval($sell_rate->amount));
                        $newFinalSchedules[$key]->sell_rates[$keySecond]->total = (floatval($sell_rate->qty) * floatval($sell_rate->amount));
       ...
       //$to = ['kenjos75@yahoo.com', 'kenjos75@gmail.com'];
        ...
        $officeFromEmails = collect([
            [
                ...
                'emails' => config('mail-settings.booking_email')
            ],
            [
        ...
        $subjects = sprintf('Booking: ID#%d - PO#,%s', $shipment->shifl_ref, $po_num_join);
        ...
        if ( isset($shipment->customer)){
            ...
            if (isset($shipment->customer->booking_email_recipients) && $shipment->customer->booking_email_recipients!==null && $shipment->customer->booking_email_recipients!=='') {
                $booking_email_recipients = $shipment->customer->booking_email_recipients;
                /*
                try {
                    $booking_email_recipients = json_decode($shipment->customer->booking_email_recipients);
                }catch(Exception $e) {
                    $booking_email_recipients = $shipment->customer->booking_email_recipients;
                }*/
        ...
        if (count($to)>0) {
            ...
            /*
            if ( $shipment->id === 19845 ) {
                $cc = [];
                $to = ['kenneth@shifl.com'];
            } */

            //get the email fields from customer
            //$email_fields = ['booking_and_updated_emails', 'booking_confirmation_emails', 'agent_booking_confirmation_emails', 'departure_notice_emails', 'arrival_notice_emails', 'delivery_order_emails', 'eta_alert_emails', 'eta_alert_trucker_emails'];
            ...
            if($shipment != null){
                if ($shipment->id != ''){
                    App\Shipment::where('id',$shipment->id)->update(['is_booking_email_sent' => '1', 'is_booking_email_sent_at' => Carbon::now()]);
                }
            }
