...
use App\Mail\SendNewBookingEmail;
...
class SendNewBookingEmailEventListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  SendNewBookingEmailEvent  $event
     * @return void
     */
    public function handle(SendNewBookingEmailEvent $event)
    {
        //\Debugbar::info('hello');
       ...
       //$to = ['kenjos75@yahoo.com', 'kenjos75@gmail.com'];
        ...
        $subjects = sprintf('Schedule Request: ID#%d - PO#,%s', $shipment->shifl_ref, $po_num_join);
        ...
        if ( isset($shipment->customer)){
            ...
            if (isset($shipment->customer->manager)) {
                array_push($to, (is_object($shipment->customer->manager)) ? $shipment->customer->manager->email : $shipment->customer->manager);
        ...
        if (count($to)>0) {
            ...
            if ( isset($shipment->officeFrom->booking_and_update_emails) ) {
                $dynamic_emails = $shipment->officeFrom->booking_and_update_emails;
            ...
            if ( isset($shipment->officeTo->booking_and_update_emails) ) {
                $dynamic_emails = $shipment->officeTo->booking_and_update_emails;
            ...
            Mail::to($to)->cc($cc)->send(new SendNewBookingEmail($subjects, $content, $attachment));
            //Mail::to($to)->cc($cc)->send(new SendNewBookingEmail($subjects, $content, $attachment));
        }
