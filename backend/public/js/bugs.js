let appAngular = angular.module('bugs-app', []);
...
appAngular.controller('BugsController',[ '$scope','$http','$timeout','$interval',function($scope,$http,$timeout, $interval) {
		...
		if (getQueue.length > 0) {
			...
			function repeat(i) {
				...
				if (getQueue[i]) {
					...
					{
					 	method: 'GET',
					 	url: `${ajax_url}/custom/process-bug-shipment/${getQueue[i].id}`
					})
	...
	$scope.processShipment = function() {
		$http(
			{
		 	method: 'GET',
		 	url: `${ajax_url}/custom/get-all-bugs-shipment?page=${$scope.page}`})
	    .then(function (response) {
