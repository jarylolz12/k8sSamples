let appAngular = angular.module('quickbooks-notifications-app', []);
...
appAngular.controller('PushNotificationController',[ '$scope','$http','$timeout','$interval',function($scope,$http,$timeout, $interval) {
		...
		if (getQueue.length > 0) {
			...
			function repeat(i) {
				...
				if (getQueue[i]) {
					...
					{
					 	method: 'GET',
					 	url: `${ajax_url}/custom/process-single-notification/${getQueue[i].id}`
					})
	...
	$scope.processNotifications = function() {
		$http(
			{
		 	method: 'GET',
		 	url: `${ajax_url}/custom/process-get-all-notifications?page=${$scope.page}`})
	    .then(function (response) {
