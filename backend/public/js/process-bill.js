let appAngular = angular.module('process-bill-app', []);
...
appAngular.controller('BillController',[ '$scope','$http','$timeout','$interval',function($scope,$http,$timeout, $interval) {
		...
		if (getQueue.length > 0) {
			...
			function repeat(i) {
				...
				if (getQueue[i]) {
					...
					{
					 	method: 'GET',
					 	url: `${ajax_url}/custom/process-single-bill/${getQueue[i].id}`
					})
	...
	$scope.processBills = function() {
		$http(
			{
		 	method: 'GET',
		 	url: `${ajax_url}/custom/process-get-all-bills?page=${$scope.page}`})
	    .then(function (response) {
