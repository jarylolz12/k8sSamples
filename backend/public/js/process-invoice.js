let appAngular = angular.module('process-invoice-app', []);
...
appAngular.controller('InvoiceController',[ '$scope','$http','$timeout','$interval',function($scope,$http,$timeout, $interval) {
		...
		if (getQueue.length > 0) {
			...
			function repeat(i) {
				...
				if (getQueue[i]) {
					...
					{
					 	method: 'GET',
					 	url: `${ajax_url}/custom/process-single-invoice/${getQueue[i].id}`
					})
	...
	$scope.processInvoices = function() {
		$http(
			{
		 	method: 'GET',
		 	url: `${ajax_url}/custom/process-get-all-invoices?page=${$scope.page}`})
	    .then(function (response) {
