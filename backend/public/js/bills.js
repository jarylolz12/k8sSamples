let appAngular = angular.module('bills-app', []);
...
appAngular.controller('BillsController',[ '$scope','$http','$timeout','$interval',function($scope,$http,$timeout, $interval) {
		...
		if (getQueue.length > 0) {
			...
			function repeat(i) {
				...
				if (getQueue[i]) {
					...
					{
					 	method: 'GET',
					 	url: `${ajax_url}/custom/process-correct-bill/${getQueue[i].id}`
					})
	...
	$scope.processBill = function() {
		$http(
			{
		 	method: 'GET',
		 	url: `${ajax_url}/custom/get-all-process-bills?page=${$scope.page}`})
	    .then(function (response) {
