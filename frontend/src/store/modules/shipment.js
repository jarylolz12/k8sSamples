...
const state = {
	...
	shipmentWithoutMblNumber: false,
	orders: [],
	ordersLoading: false,
	shipmentCancel: false,
	archivedBookings: [],
	archivedBookingsLoading: false,
	markShipmentCompletedLoading: false,
	shipmentUnArchive: false,
	createBulkShipmentLoading: false,
	updateShipmentFields: false,
	updateShipmentInfoLoading: false,
};
...
const getters = {
	...
	getCancelShipmentLoading: (state) => state.shipmentCancel,
	getArchivedBookingsLoading: (state) => state.archivedBookingsLoading,
	getArchivedBookings: (state) => state.archivedBookings,
	getmarkShipmentCompletedLoading: (state) =>state.markShipmentCompletedLoading,
	getUnArchiveShipmentLoading: (state) => state.shipmentUnArchive,
	getCreateBulkShipmentLoading: (state) => state.createBulkShipmentLoading,
	getUpdateShipmentFieldsLoading: (state) => state.updateShipmentFields,
	getCreateShipmentWithoutMBLNumber: (state) => state.shipmentWithoutMblNumber,
	getUpdateShipmentInfoLoading: (state) => state.updateShipmentInfoLoading,
};

const actions = {
	markBookExternal: async ({ commit }, payload) => {
		commit("SET_EDIT_SHIPMENT_LOADING", true);
		return new Promise((resolve, reject) => {
			function proceed() {
				let sm = new ShipmentModel(betaBaseUrl);
				sm.markBookExternal(payload)
					.then((res) => {
	...
	updateShipmentInfo: async ({ commit }, payload) => {
		commit("SET_UPDATE_SHIPMENT_INFO_LOADING", true);
		return new Promise((resolve, reject) => {
			function proceed() {
				let sm = new ShipmentModel(betaBaseUrl);
				sm.updateShipmentInfo(payload)
					.then((res) => {
						commit("SET_UPDATE_SHIPMENT_INFO_LOADING", false);
						resolve(res);
					})
					.catch((err) => {
						commit("SET_UPDATE_SHIPMENT_INFO_LOADING", false);
	...
	createShipmentWithoutMblNumber: async ({ commit }, payload) => {
		commit("SET_CREATE_SHIPMENT_WITHOUT_MBL_LOADING", true);
		return axios
			.post(`${betaBaseUrl}/v2/shipment/create-without-mbl`, payload)
			.catch((err) => {
				if (typeof err.error !== "undefined") {
					return Promise.reject(err.error);
				} else {
			...
			.finally(() => {
				commit("SET_CREATE_SHIPMENT_WITHOUT_MBL_LOADING", false);
			});
	},
	fetchOrders: async ({ commit }, payload) => {
		...
		await axios
			.get(
				`${poBaseUrl}/orders?type=${payload.orderType}&entity_id=${payload.customer_id}`
			)
			.then((res) => {
				if (res.status === 200) {
					if (typeof res.data !== "undefined") {
						commit("SET_ORDERS_VALUE", res.data.orders);
					}
				}
			})
			.catch((err) => {
	...
	cancelShipment: async ({ commit }, payload) => {
		commit("SET_CANCEL_SHIPMENT_LOADING", true);
		return axios
			.post(`${betaBaseUrl}/v2/shipment/cancel`, payload)
			.catch((err) => {
				if (typeof err.error !== "undefined") {
					return Promise.reject(err.error);
				} else {
			...
			.finally(() => {
				commit("SET_CANCEL_SHIPMENT_LOADING", false);
			});
	},
	fetchArchivedBookings: async ({ commit }, payload) => {
		commit("SET_ARCHIVED_BOOKINGS", []);
		commit("SET_ARCHIVED_BOOKINGS_LOADING", true);
		let attempt = false;
		return new Promise((resolve, reject) => {
			function proceed() {
				axios
					.get(`/v2/bookings/archived?shipmentType=${payload.tab}&search=${payload.search}&per_page=30`)
					// .get(`/v2/bookings/archived?page=${page}&per_page=30`)
					.then((res) => {
						if (res.status === 200) {
							if (typeof res.data !== "undefined") {
								let archivedBookings = res.data.data;
								let metaData = res.data.meta;
								const archivedBookingsObj = new Shipment(archivedBookings);
								...
								let finalData = {
									...
									total: metaData.total,
									last_page: metaData.last_page,
									shipments: archivedBookingsObj.draft(),
								};
								commit("SET_ARCHIVED_BOOKINGS", finalData);
							}
						} else {
							commit("SET_ARCHIVED_BOOKINGS", []);
						}
						commit("SET_ARCHIVED_BOOKINGS_LOADING", false);
						resolve();
					})
					.catch((err) => {
						if (typeof err.message !== "undefined") {
	...
	markShipmentCompleted: async ({ commit }, shipmentId) => {
		commit("SET_MARK_SHIPMENT_COMPLETED_LOADING", true);
		return axios
			.post(
				`${betaBaseUrl}/v2/shipment/mark-shipment-completed/${shipmentId}`)
			.catch((err) => {
				if (typeof err.error !== "undefined") {
					return Promise.reject(err.error);
				} else {
			...
			.finally(() => {
				commit("SET_MARK_SHIPMENT_COMPLETED_LOADING", false);
			});
	},
	unArchiveShipment: async ({ commit }, payload) => {
		commit("SET_UNARCHIVE_SHIPMENT_LOADING", true);
		return axios
			.post(`${betaBaseUrl}/v2/shipment/unarchive`, payload)
			.catch((err) => {
				if (typeof err.error !== "undefined") {
					return Promise.reject(err.error);
				} else {
			...
			.finally(() => {
				commit("SET_UNARCHIVE_SHIPMENT_LOADING", false);
			});
	},
	createBulkShipments: async ({ commit }, payload) => {
		commit("SET_CREATE_BULK_SHIPMENT_LOADING", true);
		return new Promise((resolve, reject) => {
			function proceed() {
				let sm = new ShipmentModel(betaBaseUrl);
				sm.createBulkShipment(payload)
					.then((res) => {
						commit("SET_CREATE_BULK_SHIPMENT_LOADING", false);
						resolve(res);
					})
					.catch((err) => {
						commit("SET_CREATE_BULK_SHIPMENT_LOADING", false);
	...
	updateShipmentFields: async ({ commit }, payload) => {
		commit("SET_UPDATE_SHIPMENT_FIELD_LOADING", true);
		return axios
			.post(`${betaBaseUrl}/v2/shipment/update-fields`, payload)
			.catch((err) => {
				if (typeof err.error !== "undefined") {
					return Promise.reject(err.error);
				} else {
			...
			.finally(() => {
				commit("SET_UPDATE_SHIPMENT_FIELD_LOADING", false);
			});
	},
};
const mutations = {
	...
	SET_CREATE_SHIPMENT_WITHOUT_MBL_LOADING: (state, payload) => {
		state.shipmentWithoutMblNumber = payload;
	},
	SET_ORDERS_VALUE: (state, payload) => {
		state.orders = payload;
	},
	SET_ORDERS_LOADING: (state, payload) => {
		state.ordersLoading = payload;
	},
	SET_CANCEL_SHIPMENT_LOADING: (state, payload) => {
		state.shipmentCancel = payload;
	},
	SET_ARCHIVED_BOOKINGS_LOADING: (state, payload) => {
		state.archivedBookingsLoading = payload;
	},
	SET_ARCHIVED_BOOKINGS: (state, payload) => {
		state.archivedBookings = payload;
	},
	SET_MARK_SHIPMENT_COMPLETED_LOADING: (state, payload) => {
		state.markShipmentCompletedLoading = payload;
	},
	SET_UNARCHIVE_SHIPMENT_LOADING: (state, payload) => {
		state.shipmentUnArchive = payload;
	},
	SET_CREATE_BULK_SHIPMENT_LOADING: (state, payload) => {
		state.createBulkShipmentLoading = payload;
	},
	SET_UPDATE_SHIPMENT_FIELD_LOADING: (state, payload) => {
		state.updateShipmentFields = payload;
	},
	SET_UPDATE_SHIPMENT_INFO_LOADING: (state, payload) => {
		state.updateShipmentInfoLoading = payload;
	},
};
