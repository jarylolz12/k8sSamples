				...
				<div class="d-md-flex d-sm-flex justify-space-between align-center report-details-header">
					<div class="px-3 px-sm-0">
						<span class="report-title text-h5">
							<h2 class="report-title">Profit and Loss</h2>
						</span>
					</div>
				</div>
			</div>
				...
				<v-row>
					<v-col cols="12" md="12" lg="12" class="pb-0" v-if="reportsData">
						<v-simple-table class="mb-12">
							<thead>
								<tr v-if="reportsData.tableHeader">
									<th>DESCRIPTION</th>
									<th v-for="header, i in reportsData.tableHeader" :key="i" class="text-end" style="width: 15%; min-width: 15%;"> {{ header }} </th>
									<th class="text-end" style="width: 12%; min-width: 12%;">Total</th>
								</tr>
							</thead>
							<tbody v-for="item,i in reportsData.tableBody" :key="i" class="table-body-common">
								...
								<tr v-for="footerItem, fI in item.footer" :key="'b'+fI">
									<td class="font-semi-bold text-uppercase font-12">Total</td>
									<td v-for="colItem, fI2 in footerItem.value" :key="'c'+fI2" class="text-end"> {{ colItem }}</td>
									<td class="text-end">{{ footerItem.total }}</td>
								</tr>
							</tbody>
						</v-simple-table>
					</v-col>
		...
		methods: {
			async loadReport() {
				...
				try {
                    ...
                    let reportsData = [];
                    if(report && report.class === "App\\Reports\\ProfitLoss"){
                        const data = report.data;
                        let mainHeader = data['dates'];
						let tableData = [];
