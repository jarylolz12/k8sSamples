		...
		<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
			<v-card class="invoice-delete-modal-accounting" :loading="isDeleting">
				<v-card-text class="pa-8">
					...
					<h1 class="header-1 my-4">{{ 'Delete Invoice' }}</h1>
					<p>{{ 'Do you want to delete the invoice' }} <span>"{{ formData.document_number }}"</span>?</p>
...
export default {
	...
	methods: {
		...mapActions('accounting', ['deleteInvoice']),
		...globalMethods,
		async onDelete() {
			if (this.isDeleting) {
				return;
			...
			try {
				const { data } = await this.deleteInvoice(this.formData.id);
