<template>
	<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
		<v-card class="item-disable-modal-accounting">
			<v-card-text class="pa-8">
				<v-icon size="52" color="#EB9B26">mdi-alert-circle-outline</v-icon>
				<h1 class="header-1 my-4">{{ 'Disable Item' }}</h1>
				<p>{{ 'Do you want to disable the item' }} <span>"{{ formData.name }}"</span>?</p>
...
export default {
	...
	methods: {
		...mapActions('accounting', ['disableItem']),
		...globalMethods,
		async onDelete() {
			if (this.isDeleting) {
				return;
			...
			try {
				const { message } = await this.disableItem(this.formData.id);
