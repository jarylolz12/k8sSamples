<template>
	<div class="text-center">
		<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
			<v-card class="accounting-delete-modal">
				<v-card-text class="pa-8">
					<v-icon size="52" color="#EB9B26">mdi-alert-circle-outline</v-icon>
					<h1 class="header-1 my-4">{{ header ? header : 'Confirm Action!' }}</h1>
					<p>{{ message ? message : 'Are sure you want to proceed?'}}</p>
...
export default {
	name: 'TransactionDeleteModal',
	
	props: ['open', 'formData', 'header', 'message'],
	...
	methods: {
		...mapActions('accounting', ['deleteTransaction']),
		...globalMethods,
		async onDelete() {
			if (this.isDeleting) {
				return;
			...
			try {
				const { message } = await this.deleteTransaction(this.formData);
