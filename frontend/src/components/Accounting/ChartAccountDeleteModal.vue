<template>
	<div class="text-center">
		<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
			<v-card :loading="isDeleting" class="chart-delete-modal-accounting">
				<v-card-text class="pa-8">
					<v-icon size="52" color="#EB9B26">mdi-alert-circle-outline</v-icon>
					<h1 class="header-1 my-4">{{ 'Delete Chart of Account' }}</h1>
					<p>{{ 'Do you want to delete the' }} <span>"{{ formData ? formData.trans_name : '' }}"</span>?</p>
...
export default {
	...
	methods: {
		...mapActions('accounting', ['deleteCoaAccount']),
		...globalMethods,
		async onDelete() {
			if (this.isDeleting) {
				return;
			...
			try {
				const { data } = await this.deleteCoaAccount(this.formData.id);
