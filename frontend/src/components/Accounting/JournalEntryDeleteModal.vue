<template>
	<div class="text-center">
		<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
			<v-card class="invoice-delete-modal-accounting" :loading="isDeleting">
				<v-card-text class="pa-8">
					<v-icon size="52" color="#EB9B26">mdi-alert-circle-outline</v-icon>
					<h1 class="header-1 my-4">{{ 'Delete Journal Entry' }}</h1>
					<p>{{ 'Are you sure you want to delete this journal entry' }} <span>{{' ' + formData.journal_number ? formData.journal_number : ' '}}</span>?</p>
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions('accounting', ['deleteJournalEntry']),
			...
			async onDelete() {
				...
				try {
					const { data } = await this.deleteJournalEntry(this.formData.id);
...
.invoice-delete-modal-accounting {
	.v-card__text {
