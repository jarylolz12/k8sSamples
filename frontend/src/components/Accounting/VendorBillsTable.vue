<template>
	<v-sheet>
		<v-data-table
					...
					<div class="d-md-flex justify-md-end my-2 my-md-0 mx-md-0">
						<div class="d-flex">
							<div class="search-field-wrapper">
								<Search :inputData.sync="searchText" placeholder="Search Bill #" />
							</div>
						</div>
					</div>
				</div>
			...
			<template v-slot:[`item.actions`]="{ item }">
				<v-menu offset-y bottom left content-class="outbound-lists-menu">
					...
					<v-list>
						<v-list-item link @click.prevent="$router.push(`/accounting/bills/${item.id}`)">
							<v-list-item-title>View</v-list-item-title>
						</v-list-item>
						<v-list-item link @click="onSelectEditBill(item)">
			...
			<template v-slot:no-data>
                <div class="loading-wrapper pt-6" v-if="isBillLoading">
                    <v-progress-circular
                        :size="40"
                        color="#0171a1"
                        indeterminate>
                ...
                <div class="no-data-wrapper pt-6" style="min-height: 200px;" v-if="!isBillLoading && tableData.length == 0">
					<h3 class="font-20 mb-1"> No Bills found. </h3>
					<p> There are no bills listed yet. </p>
                </div>
            </template>
		</v-data-table>
		...
		<billing-form
			...
			:form-values="selectedBillData"
			@toggle="onToggleBillForm"
		></billing-form>
	</v-sheet>
</template>
...
export default {
	...
	computed: {
		...
		pagination: {
			get() {
				return this.billsData ? this.billsData.meta : {};
			}
		}
	},
	...
	methods: {
		...globalMethods,
		async fetchData() {
			if (this.isBillsLoading) {
				return;
			}
			this.showFilter = false;
			this.isBillsLoading = true;
			try {
				const data = await AkauntingService.getVendorBills(this.contactId,{
		...
		onSelectEditBill(bill){
			this.isEditMode = true;
			this.selectedBillData = JSON.parse(JSON.stringify(bill));
			this.showFormDialog = true;
		},

		onToggleBillForm(options = {}) {
