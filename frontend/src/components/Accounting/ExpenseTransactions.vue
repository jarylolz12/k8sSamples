	...
	<v-sheet>
		...
		>
			...
			<template v-slot:[`item.number`]="{ item }">
				<a href="#" style="text-decoration: none;" @click.prevent="$router.push(`/accounting/transactions/${item.id}`)">
			...
			<template v-slot:[`item.actions`]="{ item }">
				<v-menu offset-y bottom left content-class="outbound-lists-menu">
					...
					<v-list>
						<v-list-item link @click.prevent="$router.push(`/accounting/transactions/${item.id}`)">
							<v-list-item-title>View</v-list-item-title>
						</v-list-item>
						<v-list-item link @click="onSelectEditPaymentData(item)">
							<v-list-item-title>Edit</v-list-item-title>
						</v-list-item>
						<v-list-item link @click="onSelectDeletePaymentData(item)">
							<v-list-item-title class="text-danger red--text">Delete</v-list-item-title>
						</v-list-item>
					</v-list>
				</v-menu>
			</template>
			...
			<template v-slot:no-data>
				...
				<v-alert class="mt-2" v-if="!isPaymentsDataLoading && tableData.length === 0 && searchText === ''">
					...
					<p class="subtitle-2 no-data-title">Add Bill Payment</p>
...
import TransactionDeleteModal from './TransactionDeleteModal.vue';
import globalMethods from '../../utils/globalMethods';

export default {
	name: 'AccountingExpenseTransactions',
	components: {
		Search,
		Pagination,
		BillPaymentForm,
	...
	computed: {
		...
		pagination: {
			get() {
				return this.tableData.data ? this.tableData.meta.pagination : {};
			}
		}
	},
	...
	methods: {
		...mapActions('accounting', ['getBillPaymentData']),
		...globalMethods,
		async fetchData() {
			...
			try {
				...
				const data = await this.getBillPaymentData({
		...
		onSelectDeletePaymentData(data = null) {
		...
		onToggleDeleteModal(options = null){
			...
			if (options.deleted) {
				...
				const message = options.message || 'Bill Payment has been deleted.'
