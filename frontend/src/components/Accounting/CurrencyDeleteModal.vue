<template>
	<div class="text-center">
		<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
			<v-card class="currency-delete-modal-accounting">
				<v-card-text class="pa-8">
					<v-icon size="52" color="#EB9B26">mdi-alert-circle-outline</v-icon>
					<h1 class="header-1 my-4">{{ 'Delete Currency' }}</h1>
					<p>{{ 'Delete the currency' }} <span>"{{ formData.name }}"</span>?</p>
...
export default {
	...
	methods: {
		...mapActions('accounting', ['deleteCurrency']),
		...globalMethods,
		async onDelete() {
			if (this.isDeleting) {
				return;
			...
			try {
				const { message } = await this.deleteCurrency(this.formData.id);
...
.currency-delete-modal-accounting {
	.v-card__text {
