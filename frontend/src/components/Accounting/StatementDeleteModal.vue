<template>
	<div class="text-center">
		<v-dialog v-model="showDialog" max-width="480" origin="top center" content-class="accounting-module-dialog pa-0" persistent scrollable>
			<v-card class="customer-delete-modal-accounting">
				<v-card-text class="pa-8">
					<v-icon size="52" color="#EB9B26">mdi-alert-circle-outline</v-icon>
					<h1 class="header-1 my-4">{{ 'Delete Statement' }}</h1>
					<p>{{ 'Are you sure want to delete the selected statement?' }}?</p>
	...
	export default {
		...
		methods: {
			...
			async onDelete() {
				...
				try {
					const { message } = await AkauntingService.deleteStatement(this.formData.id);
...
.customer-delete-modal-accounting {
	.v-card__text {
