                        ...
                        <div class="product-info">
                            ...
                            <div class="product-contact mb-2" v-if="checkIfHasChildProducts(productItem)">
                                ...
                                <v-select
                                    ...
                                    :items="[...contactListsSelected, ...contactItemsCopy]"
                                    ...
                                    <template v-slot:item="{ item, attrs, on }">
                                        <v-list-item v-on="on" v-bind="attrs" #default="{ active }" @click.stop="ifIsContactSelected(item)"
                                            :class="typeof item.customClass === 'undefined' ? '' : item.customClass">
...
export default {
    ...
    props: ['dialog', 'editedIndex', 'categoryLists', 'isMobile',
            ...
            'isWarehouse3PLProvider', 'searchVal', 'searchFromInventory', 'productsData', 'inboundItems', 'contactItems'],
    components: {},
    data: () => ({
        categoryListData:[],
    ...
    methods: {
        ...
        handleSearchContact() {
            if (!this.searchContactName) {
                this.contactItemsCopy = this.contactItems;
            }

            this.contactItemsCopy = this.contactItems.filter(c => {
        ...
        removeChip(item) {
            ...
            if (this.editedIndex === -1) {
                ...
                // indexLodash = _.findIndex(this.contactListsSelected, e => (e.contact_id == item.contact_id))
