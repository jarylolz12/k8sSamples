...
<template>
	<div class="po-details-wrapper-table shipments">
		<div>
			<v-data-table
				:headers="headers"
				:items="poDetailsShipments"
				class="elevation-1"
				:class="
					poDetailsShipments !== null &&
					poDetailsShipments.length !== 'undefined' &&
					poDetailsShipments.length > 0
						? 'po-details-table'
						: 'no-data-table'
				"
				hide-default-footer
							...
							<td>
								...
								>
									<div class="no-data-heading">
										...
										<h3>No Shipments</h3>
										<p>
											There are no shipments associated in this PO yet.
										</p>
									</div>
								</div>
							</td>
				...
				>
					...
					<template v-slot:no-data>
						...
						<div
							class="no-data-wrapper"
							v-if="
								!getPoShipmentDetailsLoading && poDetailsShipments.length === 0
							"
						>
							<div class="no-data-heading">
								<img
...
export default {
	...
	computed: {
		...mapGetters({
			...
			getPoShipmentDetails: "po/getPoShipmentDetails",
			getPoShipmentDetailsLoading: "po/getPoShipmentDetailsLoading",
		}),
		poDetailsShipments() {
			let poShipments = [];
				...
				this.getPoShipmentDetails.data.map((v) => {
					if (v.purchase_order_data !== null) {
						let products = v.purchase_order_data.purchase_order_products.map(
							(p) => {
								// let { id, product, units_per_carton, shipment_distribution } = p
