<template>
    <div class="po-table-wrapper">
        <v-data-table
            ...
            :class="(posShipped !== null &&
                    posShipped.length > 0) ? '' : 'no-data-table'"
            hide-default-footer
            fixed-header
            @click:row="viewPo"
            show-select>
            ...
            <template v-slot:no-data>
                ...
                <div class="no-data-wrapper" v-if="!getPoShippedLoading && posShipped.length == 0">
                    <div class="no-data-heading">
                        <img src="@/assets/icons/empty-po-icon.svg" width="40px" height="42px" alt="">
                        <h3>{{ (getPoLocalQuery!=='' && getPoLocalQuery!==null) ? 'No matching result': 'No Shipped Purchase Orders' }} </h3>
                        <p v-if="(getPoLocalQuery=='' || getPoLocalQuery==null)">
                            {{ "There are no purchase order to show in shipped."}} <br>
                        </p>
                        <p v-if="(getPoLocalQuery!=='' & getPoLocalQuery!==null)">
                        ...
                        <div v-if="(getPoLocalQuery!==null && getPoLocalQuery!=='')" class="mt-4">
                            <a dark color="primary" class="btn-white search-local different-btn">
