<template>
    <div class="po-table-wrapper">
        <v-data-table
            ...
            v-bind:class="{
                'no-data-table' : (typeof posPending !== 'undefined' && posPending.length === 0 && this.search === ''),
                'no-current-pagination' : (posPending.length !== 0 && getTotalPage <= 1),
            ...
            <template v-slot:[`item.status`]="{ item }">
                <div :class="item.state">
                    <!-- <v-chip> 
                        {{ item.status !== null ? 
                            (item.status === 'partially shipped' ? 'Partially Booked' : item.status) : '' 
                        }} 
                    </v-chip> -->
                    <v-chip> {{ item.state !== null ? item.state : "" }} </v-chip>
                </div>
            </template>
                ...
                <div class="actions">
                    <v-menu bottom offset-y left content-class="po-lists-menu">
                        ...
                        <v-list class="po-lists">
                            ...
                            <v-list-item @click="deletePo(item)" v-if="item.status === 'pending'">
                                <v-list-item-title>
                                   <div class="title-img">
            ...
            <template v-slot:no-data>
                ...
                <div class="no-data-wrapper" v-if="!getAllPosLoading && posPending.length == 0">
                    <div class="no-data-heading">
                        <img src="@/assets/icons/empty-po-icon.svg" width="40px" height="42px" alt="">

                        <h3>{{ (getPoLocalQuery!=='' && getPoLocalQuery!==null) ? 'No matching result': 'Create Purchase Orders' }}</h3>
                        <p v-if="(getPoLocalQuery==null || getPoLocalQuery=='')">
                            {{ "There is no purchase order to show." }}
                            <br> {{ "Let’s create a new purchase order." }}
                        </p>
                        <p v-if="(getPoLocalQuery!=='' && getPoLocalQuery!==null)">
                            {{ "Sorry! We could not find any purchase order that matches your search term." }}
                        </p>
                        <div v-if="(getPoLocalQuery==null || getPoLocalQuery=='')" class="po-table-button-empty-wrapper">
        ...
        <Pagination v-if="typeof posPending !== 'undefined' && posPending.length > 0 && getTotalPage > 1"
...
export default {
    ...
    computed: {
        ...
        getTotalPage: {
            get() {
                return Math.ceil(this.getPendingPOPagination.total / this.getPendingPOPagination.per_page)
            }
        },
        getCurrentPage: {
            get() {
        ...
        getSearchedDataClass() {
            if (this.posPending.length == 0 && this.search !== '') {
                return true
            } else {
                return false
            }
        ...
        // headers(){
            // let state = this.posPending.map( item => 
            // {
            //     return item.state;
            // });
