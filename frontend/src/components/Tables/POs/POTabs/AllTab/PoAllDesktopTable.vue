...
<template>
	<div class="po-table-wrapper">
		...
		>
			...
			<template v-slot:[`item.vendor_ship_to`]="{ item }">
				<div>
					{{
						item.supplier_id !== null
							? getSupplierName(item.supplier_id)
			...
			<template v-slot:[`item.forwarder`]="{ item }">
				<span class="noSpace" :class="item.id">{{ item.forwarder }}</span>
			</template>
			...
			<template v-slot:[`item.etd`]="{ item }">
				<span class="noSpace">{{ formatDate(item.etd) }}</span>
			</template>
					...
					<v-menu bottom offset-y left content-class="po-lists-menu">
						...
						<v-list class="po-lists">
							<v-list-item
								...
								v-if="
									(item.state == 'approved' ||
										...
										!item.mark_as_paid
								"
								@click="markAsPaidDialog(item, '1')"
							>
								<v-list-item-title class="open-state-action pt-2 pb-3">
									<span class="text-blue">Mark as Paid</span>
								</v-list-item-title>
							</v-list-item>
							...
							>
								<v-list-item-content>
									<v-list-item-title>
										<span class="text-note">Marked as Paid </span>
										<img class="ml-2" src="@/assets/icons/view-eye.svg" />
									</v-list-item-title>
								...
								v-if="
									item.mark_as_paid &&
										(item.mark_as_paid.status == 1 ||
											item.mark_as_paid.status == 2)
								"
								@click="markAsPaidDialog(item, '2')"
							>
								<v-list-item-title class="open-state-action pb-3">
									<span class="text-blue">Re-upload Proof</span>
								</v-list-item-title>
							</v-list-item>
							...
							>
								<v-list-item-title class="open-state-action pt-2 pb-3">
									<span class="text-blue">Submit PO</span>
								</v-list-item-title>
							</v-list-item>
							...
							<!-- <v-list-item v-if="item.state == 'approved'">
								<v-list-item-title class="open-state-action pt-2 pb-3 mb-1">
							...
							>
								<v-list-item-title>
									<span :class="item.state !== 'approved' ? 'text-blue' : ''"
							...
							>
								<v-list-item-title>
									<span class="text-red">Cancel PO</span>
								</v-list-item-title>
							</v-list-item>
						</v-list>
					</v-menu>
...
export default {
	...
	computed: {
		...mapGetters({
			...
			getAllPosPagination: "po/getAllPosPagination",
		...
		getCurrentPage: {
			get() {
				...
				// return this.getAllPosPagination.current_page
			},
			set() {
				return {};
			},
	...
	methods: {
		...
		checkStoreKey(pageNumber) {
			let poObject = this.$store.state.po.allPosData;
