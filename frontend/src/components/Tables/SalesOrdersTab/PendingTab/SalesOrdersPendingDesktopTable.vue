...
<template>
	<div class="po-table-wrapper">
		...
		>
			...
			<template v-slot:[`item.cargo_ready`]="{ item }">
				<span class="noSpace">{{ item.cargo_ready_date }}</span>
			</template>
...
export default {
	name: "SalesOrdersPendingDesktopTable",
	props: [
		"items",
	...
	methods: {
		...
		checkboxValue(value) {
			this.selectedRows = value.map((item) => item.order_number);
		},
	},
	mounted() {},
	updated() {},
