		...
		>
			...
			<template v-slot:no-data>
				...
				<div
					...
					v-if="!getPoShippedLoading && posShipped.length == 0"
				>
					<div class="no-data-heading">
						...
						<h3>
							{{
								getSoLocalQuery !== "" && getSoLocalQuery !== null
									? "No matching result"
									: "No Shipped Purchase Orders"
							}}
						</h3>
						<p v-if="getSoLocalQuery == '' || getSoLocalQuery == null">
							{{ "There are no sales order to show in shipped." }} <br />
						</p>
						<p v-if="(getSoLocalQuery !== '') & (getSoLocalQuery !== null)">
							{{
						...
						>
							<a
								dark
								color="primary"
								class="btn-white search-local different-btn"
							>
...
export default {
	name: "SalesOrderBookedMobileTable",
	...
	computed: {
		...mapGetters({
			...
			getPoShippedLoading: "salesOrders/getPoShippedLoading",
