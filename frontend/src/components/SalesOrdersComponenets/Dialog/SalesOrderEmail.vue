    ...
    <v-dialog v-model="dialogCreate" max-width="560px" content-class="email-dialog" :retain-focus="false" @click:outside="close">
        <v-card class="email-dialog-card">
            <v-form ref="form" v-model="valid" action="#" @submit.prevent="">
                <v-card-title>
                    <span class="headline">Email Sales Order to Buyer</span>
                        ...
                        <v-col cols="12" sm="12" class="pb-0">
                            <div class="vendor-wrapper">
                                <div class="vendor-details">
                                    <p class="email-title">Buyer</p>
                                    <p class="email-data" v-html="getBuyer(emailItem.po.buyer_id)"></p>
                                </div>
                            </div>
                        </v-col>
                    </v-row>
                ...
                <v-card-actions class="po-button-actions">
                    <button class="btn-blue" @click="sendBuyerEmail()" :disabled="getEmailLoading">
                        <span>
...
export default {
    ...
    computed: {
        ...mapGetters({
            getEmailLoading: 'salesOrders/getEmailLoading',
    ...
    methods: {
        ...mapActions({
            sendEmail: 'salesOrders/sendEmail',
        ...
        getBuyer(id) {
            if ( Array.isArray(this.getBuyerLists) && this.getBuyerLists.length > 0) {
                let findVendor = _.find(this.getBuyerLists, (e => e.id === id))
                ...
                if (this.$refs.form.validate()) {
                    ...
                    if (!this.tagsInput.hasError && isValid) {
                        try {
                            ...
                            this.notificationMessage('Sales order has been emailed to the buyer.')
