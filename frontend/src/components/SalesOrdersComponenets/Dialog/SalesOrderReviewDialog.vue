...
<template>
	<v-dialog
		...
		max-width="1440px"
		content-class="sales-order-review-dialog"
		:retain-focus="false"
		persistent
		scrollable
	>
		<v-card class="po-dialog-card">
					...
					<div class="create-wrapper">
						...
						<div class="product-info">
							...
							>
								...
								<template v-slot:[`item.buyer_sku`]="{ item }">
									<div
								...
								<template v-slot:[`item.buyer_sku_description`]="{ item }">
									<div class="product-sku-name h-100">
										<v-textarea
											...
											auto-grow
											rows="2"
											class="table-text-fields text-left mt-0"
											hide-details="auto"
											disabled
										>
										</v-textarea>
			...
			<v-card-actions class="po-button-actions">
				<button
					class="btn-blue"
					:disabled="
						getSoCreateLoading || getSoDraftLoading || !changeRequestButton
					"
					v-if="changeRequestButton || poItem.change_request_button"
					@click="savePo()"
				>
				...
				<button
					class="btn-blue"
					:disabled="getOrdersStateLoading || getSoCreateLoading"
					v-if="!changeRequestButton && !poItem.change_request_button"
				...
				<button
					class="btn-white"
					:disabled="
						getSoCreateLoading || getSoDraftLoading || getOrdersStateLoading
					"
...
export default {
	...
	data: () => ({
		...
		buyer_SKU_Decription: [
			{
				text: "Buyer’s SKU & Description",
				align: "start",
				sortable: false,
				value: "buyer_sku_description",
				fixed: true,
				width: "18%",
				sort_order: 3,
			},
	...
	computed: {
		...
		dynamicHeader() {
			...
			if (this.poItem.is_issuer) {
				dynamicHeader = [...this.headers, ...this.buyer_SKU];
			} else {
				dynamicHeader = [...this.headers, ...this.buyer_SKU_Decription];
			}
	...
	methods: {
		...
		async updateCartonCount(index) {
			...
			if (this.fromComponent == "so-details-page") {
				...
				let data = this.salesOrdersAll.find((item) => {
		...
		async orderStateUpdate(orderState) {
			...
			if (this.$refs.form.validate()) {
				if (this.performaInvoice) {
					...
					let editItemData = this.addPoTemplate;
					const payload = {
						poNumber: this.poItem.po_number,
						orderAction: orderState,
						requiredDepositType: editItemData.required_deposit_type,
						requiredDepositValue: editItemData.required_deposit_value,
						committedCargoReadyDate: editItemData.committed_cargo_ready_date,
						notes: this.poItem.notes,
						signature: this.poItem.signature,
				...
				if (this.$router.history.current.name !== "SalesOrders Details") {
		...
		changeButton(changeButtonFlag) {
			this.changeRequestButton = changeButtonFlag;
			if (this.changeRequestButton) {
				this.performaInvoice = false;
			}
		},
...
<style lang="scss">
@import "../../../assets/scss/pages_scss/salesOrders/salesOrdersReviewDialog.scss";
</style>
