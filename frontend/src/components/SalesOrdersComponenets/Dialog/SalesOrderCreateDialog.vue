...
<template>
	<v-dialog
		...
		max-width="1392px"
		content-class="sales-order-dialog"
		:retain-focus="false"
		persistent
		scrollable
	>
		<v-card class="po-dialog-card">
				...
				<v-form ref="form" v-model="valid" action="#" @submit.prevent="">
					<div class="create-wrapper">
						<div class="sales-order-info">
							<div class="section-1">
								<div class="po-vendor mb-3">
									...
									<v-autocomplete
										...
										"
										@change="fetchSuppliersSkuOnChange(poItem.buyer_id)"
									>
									</v-autocomplete>
								</div>
							...
							<div class="section-2">
								<div class="po-vendor mb-3">
									<p class="po-title">Import Name(Optional)</p>
								...
								<div class="po-ship-details mb-3">
									...
									<div class="ship-details-wrapper">
										<p class="po-title">
											Must Arrive By <small>(Optional)</small>
										</p>
						...
						<div class="product-info">
							...
							<tbody v-if="items.length > 0">
								<tr v-for="(item, index) in items" :key="item.id">
									<td :class="getSuppliersSkusLoading ? 'td-disabled' : ''">
										<div
											...
											<div class="product-mobile-wrapper" v-if="isMobile">
												<div
													...
													class="product-mobile-header d-flex justify-space-between align-center"
												>
													<p class="mb-0 po-create-product-title">
														Product {{ getProductNumber(index) }}
													</p>
													<v-btn
												...
												<v-autocomplete
													...
													:placeholder="getSuppliersSkusLoading || productLoading ? 'Fetching Products':'Select SKU'"
													:disabled="getSuppliersSkusLoading"
													outlined
													item-text="name"
													item-value="id"
													:menu-props="{
												...
												>
														...
														<div class="option-items">
															<div class="sku-item">
																...
																<div>
																	<p>{{ item.name }}</p>
																	<p
																		style="color: #6D858F !important; font-family: 'Inter-Medium', sans-serif;"
																	>
																		${{
													...
													<template v-slot:no-data>
														<div v-if="getSuppliersSkusLoading || productLoading" class="option-items loading"
														...
														<div v-if="!getSuppliersSkusLoading || !productLoading" class="option-items no-data"
									...
									<td>
										<div class="sales-ship-date-and-time-wrapper">
											<div class="sales-eta"
												v-for="(product, i) in item.product_split"
												:key="i"
												:class="i !== 0 ? 'border-top-add' : (item.product_split.length > 1 ? '' : 'first-item-so')">
												<v-text-field
													type="date"
									...
									<td>
										<div v-for="(product, i) in item.product_split"
											:key="i"
											:class="i !== 0 ? 'border-top-add' : (item.product_split.length > 1 ? '' : 'first-item-so')">
										<v-text-field
											type="number"
										...
										<span
											class="error-message"
											style="font-size: 11px; color: red;"
											v-if="checkShowErrorMessageCartons(item)"
										>
											{{
										...
										<!-- error if it detects that untis has error -->
										<span
											class="error-message"
											style="font-size: 11px; color: white;"
									...
									<td>
										...
										<span
											class="error-message"
											style="font-size: 11px; color: red;"
											v-if="checkShowErrorMessageUnits(product)"
										>
											{{
								...
								<!-- <template v-slot:[`item.volume`]="{ item, index }">
									<div class="h-100">
										...
										<span
											class="error-message"
											style="font-size: 11px; color: red;"
											v-if="checkShowErrorMessageUnits(item)"
										>
											{{
												typeof item.unitErrorMessage !== "undefined" &&
												item.unitErrorMessage !== null
			...
			<v-card-actions class="po-button-actions">
				<button
					...
					:disabled="getSoCreateLoading || getSoDraftLoading"
				>
					<span v-if="editedIndex === -1">
						{{ getSoCreateLoading ? "Creating..." : "Create & Submit" }}
					</span>
					<span v-if="editedIndex > -1">
						{{ getSoCreateLoading ? "Saving Sales Order..." : "Save & Submit" }}
					</span>
				</button>
				...
				<button
					...
					@click="savePo()"
					:disabled="getSoCreateLoading || getSoDraftLoading || fromComponent == 'BookingShipmentDialog'"
				>
					<span v-if="editedIndex === -1">
						{{
							getSoDraftLoading ? "Creating Sales Order..." : "Save to Draft"
						}}
					</span>
					<span v-if="editedIndex > -1">
						{{ getSoDraftLoading ? "Saving Sales Order..." : "Save" }}
					</span>
				</button>
...
import PoTotalComponent from "../../PosComponents/PoTotalComponent.vue";
import globalMethods from "../../../utils/globalMethods";
import _ from "lodash";
import axios from "axios";
import moment from "moment";
import SalesOrderInvalidRequest from "./SalesOrderInvalidRequest.vue";
...
export default {
	...
	computed: {
		...mapGetters({
			...
			getPoCreateLoading: "salesOrders/getPoCreateLoading",
			getSoCreateLoading: "salesOrders/getSoCreateLoading",
			getSoDraftLoading: "salesOrders/getSoDraftLoading",
			getPoSaveAddLoading: "salesOrders/getPoSaveAddLoading",
			...
			getImportName: "salesOrders/getImportName",
			getSuppliersSkus:'salesOrders/getSuppliersSkus',
			getSuppliersSkusLoading:'salesOrders/getSuppliersSkusLoading'
		}),
		...
		SuppliersSkuItems(){
			let products = []
			if(this.getSuppliersSkus !== undefined && this.getSuppliersSkus !== null && this.getSuppliersSkus !== 'undefined'){
				products = this.getSuppliersSkus	
			}else{
				products = []
			}
			return products
		...
		productSkuDropDownItems(){
			let data = []
			if(this.poItem.buyer_id !== '' && this.poItem.buyer_id !== null){
				data = this.SuppliersSkuItems
			}else{
			...
			get() {
				...
				if(typeof tempItem.products !== "undefined" &&
					tempItem.products.length !== 0){
						if(this.editedIndex !== -1) { // editing po							
		...
		buyerListOptions() {
			let newVendorLists = [];
			if (
				typeof this.getBuyerLists !== "undefined" &&
				Array.isArray(this.getBuyerLists) &&
				this.getBuyerLists.length > 0
			) {
				newVendorLists = this.getBuyerLists.map((gvl) => ({
		...
		addPoTemplate() {
			...
			let buildItem = {
				...
				committed_cargo_ready_date: committedCargoReadyDate,
		...
		buttonDisabled() {
			let button = false;
			let { buyer_id, products } = this.poItem;
	...
	methods: {
		...mapActions({
			fetchVendorLists: "salesOrders/fetchVendorLists",
			fetchBuyerLists: "salesOrders/fetchBuyerLists",
			fetchPoSelectProducts: "salesOrders/fetchPoSelectProducts",
			fetchPo: "salesOrders/fetchPo",
			createPo: "salesOrders/createPo",
		...
		...mapMutations({restProductAndSkuItemsSuppliers:'salesOrders/restProductAndSkuItemsSuppliers'}),
		...globalMethods,
		restrictValues(e) {
			if (
				(e.key == "-" && e.keyCode == 189) ||
				...
				this.fetchPoSelectProducts(value)
					.then((res) => {
						...
						if (typeof res.data !== "undefined") {
							...
							if (typeof res.data.volume !== "undefined" &&
								res.data.volume !== "" &&
							...
							if (typeof res.data.weight !== "undefined" &&
								res.data.weight !== "" &&
		...
		close() {
			...
			this.poItem.buyer_id = ''
			this.restProductAndSkuItemsSuppliers([])
			this.$emit("close");
		},
						...
						if (this.hasProductDuplicates) {
							...
							try {
								if (this.editedIndex === -1) {
									...
									this.notificationMessage("Sales Order has been created.");
									...
									this.notificationMessage("Sales Order has been updated.");
									...
									if (
										this.$router.history.current.name !== "SalesOrders Details"
									) {
										if (this.poItem.state == DRAFT) {
											if (isDraft) {
												this.setCurrentSOTab(1);
				...
				if (this.$refs.form.validate()) {
					if (this.hasProductDuplicates) {
						...
						try {
							...
							if (this.getCurrentSoTab === 0) {
								await this.fetchPoAllNew({
									page: this.getAllPoPage,
								});
							} else if (this.getCurrentSoTab === 1) {
								await this.fetchPoPendingNew({
									page: this.getPendingPage,
								});
							} else if (this.getCurrentSoTab === 2) {
		...
		checkBothErrorUnitsAndCartons(item) {
			if (item !== undefined && item !== "undefined" && item !== null) {
				if (
					(item.showErrorUnit !== "undefined" &&
						item.showErrorUnit &&
						item.hasUnitError) ||
					(item.showErrorCarton !== "undefined" &&
						item.showErrorCarton &&
						item.hasCartonError)
				) {
					return true;
				} else {
		...
		customFilter(item, queryText, itemText) {
			const text = "#" + item.sku.toLowerCase();
			const textOne = item.sku.toLowerCase();
		...
		// suppliers Sku Api
		async fetchSuppliersSkuOnChange(val){
			if(val !== null && val !== undefined){
				try{
					await this.fetchSuppliersSku(val)
				}catch(e){
					this.notificationError(e)
				}
			}
...
<style lang="scss">
@import "../../../assets/scss/pages_scss/salesOrders/salesOrdersCreateEditDialog.scss";
</style>
